          ...
          <div className="container mx-auto px-4 md:px-12 pb-12 lg:pb-24 max-w-5xl hero-text-glass">
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                <div className="inline-block px-3 py-1 bg-kph-red text-white rounded text-[8px] sm:text-[9px] font-bold uppercase tracking-widest shadow-lg border border-red-400/20">
                  {article.category}
                </div>
                {hasVideo && (
              ...
              <h1 className="font-serif text-2xl sm:text-4xl md:text-5xl lg:text-7xl text-white leading-[1.1] mb-8 font-black tracking-tight drop-shadow-2xl">
                {article.title}
              </h1>

              <div className="flex flex-wrap items-center gap-6 sm:gap-10 text-white/90">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 bg-white/10 backdrop-blur-xl border border-white/20 rounded-full flex items-center justify-center font-black text-xs sm:text-sm text-white shadow-xl">
                    {article.author.charAt(0)}
                  </div>
                  <div>
                    <p className="text-[9px] sm:text-[10px] font-bold text-white/40 uppercase tracking-widest mb-0.5">Author</p>
                    <p className="text-sm sm:text-base font-black text-white whitespace-nowrap tracking-tight">{article.author}</p>
                  </div>
                </div>

                <div className="flex items-center gap-6 sm:gap-8 text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.15em] text-white/60">
                  <span className="flex items-center gap-2"><Calendar size={14} className="text-kph-red shrink-0" /> {new Date(article.date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short' })}</span>
                  <span className="flex items-center gap-2"><Clock size={14} className="text-kph-red shrink-0" /> {article.readTime}</span>
                </div>
              </div>
            </motion.div>
          </div>
