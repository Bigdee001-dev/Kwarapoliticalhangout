                  ...
                  <div className="max-w-4xl mx-auto space-y-16">
                     <div className="space-y-6">
                        ...
                        <div className="flex flex-wrap items-center gap-8 pb-10 border-b border-gray-50">
                           ...
                           <div className="flex items-center bg-gray-50 rounded-2xl border border-gray-100 px-3 py-1.5 focus-within:border-gray-300 transition-all flex-1 min-w-[200px]">
                              <Video size={16} className="text-gray-400 mr-2 shrink-0" />
                              <input
                                 type="text"
                                 value={newArticle.videoUrl}
                                 onChange={e => setNewArticle({ ...newArticle, videoUrl: e.target.value })}
                                 placeholder="Paste YouTube or video URL..."
                                 className="bg-transparent border-none outline-none text-xs font-medium w-full text-gray-700 placeholder-gray-400"
                              />
                           </div>
                           ...
                           >
                              <CloudUpload size={18} className="text-kph-red" /> {newArticle.imageUrl || newArticle.videoUrl ? 'Rotate File' : 'Upload Cover'}
                           </button>
                     ...
                     {(newArticle.imageUrl || newArticle.videoUrl) && !isUploading && (
                        <div className="relative group rounded-[4rem] overflow-hidden shadow-2xl border border-gray-100 group">
                           {newArticle.videoUrl ? (
                              newArticle.videoUrl.includes('youtube.com') || newArticle.videoUrl.includes('youtu.be') ? (
                                 <iframe
                                    className="w-full h-[600px] bg-black"
                                    src={`https://www.youtube.com/embed/${newArticle.videoUrl.match(/(?:youtu\.be\/|youtube\.com\/(?:embed\/|v\/|watch\?v=|watch\?.+&v=))([^#&?]{11})/)?.[1]}?autoplay=0`}
                           frameBorder="0"
